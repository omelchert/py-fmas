...
from fmas.raman_response import h_LA as hR
from fmas.raman_response import h_HC as hR
...
def main():
    ...
    hRw = model.hRw #hR(grid.t)
    for i in range(grid.w.size):
      print(grid.w[i], np.real(hRw[i]))
#
    exit()
    model.hRw = hR(grid.t)
