...
class Figure():
    ...
    def set_subfig_03(self):
        ...
        l1 = axC.plot(z[:-1]*1e-4, err, color='k', linewidth=1., label=r'RK4IP ($\Delta z = 40\,\mathrm{\mu m}$)')
        ...
        z,t,w,utz, dz_int, Cp = fetch_data('../res_CQE_SC_Nz4000.h5')
        #z = z*1e-4 # micron -> cm
        dz = z[1]-z[0]
        err = np.abs(Cp[:-1]-Cp[1:])*dz_int/dz/Cp[:-1]
        l3 = axC.plot(z[:-1]*1e-4, err, color='k', linewidth=1., dashes=[2,2], label=r'CQE ($\delta_G=10^{-8}$)')

        z,t,w,utz, dz_int, Cp = fetch_data('../res_LEM_SC_Nz4000.h5')
        #z = z*1e-4 # micron -> cm
        dz = z[1]-z[0]
        err = np.abs(Cp[:-1]-Cp[1:])*dz_int/dz/Cp[:-1]
        l4 = axC.plot(z[:-1]*1e-4, err, color='k', linewidth=1., dashes=[1,1], label=r'LEM ($\delta_G=10^{-7}$)')

        set_legend(axC,l1+l3+l4,loc=1,ncol=4)
...
def main():
    ...
    myFig = Figure(aspect_ratio=1.6, fig_basename='fig_02_v2', fig_format='svg')
    myFig.set_layout()
    myFig.set_subfig_01()
    myFig.set_subfig_02()
    myFig.set_subfig_03()
