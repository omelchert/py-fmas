...
class Figure():
    ...
    def set_layout(self):
        ...
        plt.subplots_adjust(left = 0.12,    # pos. of subplots left border
                            ...
                            wspace = .6,    # horizontal space between supblots
                            hspace = 0.6    # vertical space between subplots
                            )

        gs00 = GridSpec(nrows = 1, ncols = 1)

        gsA = GridSpecFromSubplotSpec(2, 3, subplot_spec=gs00[0,0], wspace=0.1, hspace=0.1)
    ...
    def set_subfig_1(self):
        ...
        def _set_colorbar(fig, img, ax, label='text', dw=0.):
            ...
            fig.text(x0, y0+h+0.02*h, label, horizontalalignment='left', verticalalignment='bottom', size=6)
            return colorbar
    ...
    def set_subfig_2(self):
        ...
        axA2.tick_params(axis='x', length=2., pad=2, top=False, labelbottom=False)
        x_lim = (0,512)
        ...
        axA2.tick_params(axis='y', length=2., pad=2, top=False, labelleft=False, right=True, labelright=True, left=False)
        y_lim = (0,8.5)
        y_ticks = (0,2,4,6,8)
        axA2.set_ylim(y_lim)
        ...
        axA3.tick_params(axis='y', length=2., pad=2, top=False, labelleft=False, right=True, labelright=True, left=False)
        axA3.set_ylim(y_lim)
...
def main():
    ...
    myFig = Figure(aspect_ratio=0.66, fig_basename='fig_LEM_SolSolCollision', fig_format='png')
    myFig.set_layout()
